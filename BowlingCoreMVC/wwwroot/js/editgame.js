...
function ScoreGame(g)
{
    ...
    for (var i = 0; i < 10; i++)
    {
        ...
        if ((parseInt(g.Frames[i].FrameNum) == 10) && (parseInt(g.Frames[i].ThrowOneScore) + parseInt(g.Frames[i].ThrowTwoScore) < 10))
        {
            g.Frames[i].ThrowThreeScore = 0;
        }

        if (parseInt(g.Frames[i].ThrowOneScore) == 10 && parseInt(g.Frames[i].FrameNum) != 10)
        {
